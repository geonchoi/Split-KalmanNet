...
class DNN_SKalmanNet_GSS(torch.nn.Module):
    def __init__(self, x_dim:int=2, y_dim:int=2):
        ...
        # self.input_dim_1 = (self.x_dim) * 1 + (self.y_dim) + (self.x_dim * self.y_dim)
        self.input_dim_2 = (self.y_dim) * 2 + (self.y_dim) + (self.x_dim * self.y_dim)
        # self.input_dim_2 = (self.y_dim) * 1 + (self.y_dim) + (self.x_dim * self.y_dim)
    ...
    def forward(self, state_inno, observation_inno, diff_state, diff_obs, linearization_error, Jacobian):
        ...
        # input1 = torch.cat((state_inno, linearization_error, Jacobian), axis=0).reshape(-1)
        # input1 = state_inno.reshape(-1)
        ...
        # input2 = torch.cat((observation_inno, linearization_error, Jacobian), axis=0).reshape(-1)
        # input2 = observation_inno.reshape(-1)
