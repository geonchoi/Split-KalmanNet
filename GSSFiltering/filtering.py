...
class KalmanNet_Filter():
    ...
    def filtering(self, observation):
        ...
        if is_cov:
            H_jacob = self.GSSModel.Jacobian_g(x_predict)        
            H_tilde = torch.linalg.inv(torch.transpose(H_jacob, 0, 1) @ H_jacob)
            tmp1 = torch.eye(self.x_dim) - K_gain@H_jacob
            tmp2 = torch.linalg.inv(torch.eye(self.y_dim) - H_jacob@K_gain)
            cov_post = (tmp1)@H_tilde@torch.transpose(H_jacob,0,1)@tmp2@H_jacob@K_gain@H_jacob@H_tilde
            self.cov_trace_history = torch.cat((self.cov_trace_history, torch.trace(cov_post).reshape(-1).clone()))
...
class Split_KalmanNet_Filter():
    ...
    def filtering(self, observation):
        ...
        if is_cov:
            ...
            tmp2 = torch.linalg.inv(torch.eye(self.y_dim) - H_jacob@K_gain)
            cov_post = (tmp1)@H_tilde@torch.transpose(H_jacob,0,1)@tmp2@H_jacob@K_gain@H_jacob@H_tilde      
            self.cov_post_list += [torch.trace(cov_post).item()]  
            self.Pk_list += [torch.trace(Pk).item()]
            self.Sk_list += [torch.trace(Sk).item()]

            self.cov_post2_list += [torch.abs(torch.trace((torch.eye(self.x_dim) - K_gain @ H_jacob) @ Pk))]    
