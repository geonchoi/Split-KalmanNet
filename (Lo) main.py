from GSSFiltering.model import LorenzAttractor
...
# if TRAIN:
#     LorenzAttractor(mode='train').generate_data()
#     LorenzAttractor(mode='valid').generate_data()
# LorenzAttractor(mode='test').generate_data()
...
if TRAIN:
    ...
    trainer_kalman = Trainer(
        dnn=KalmanNet_Filter(
            LorenzAttractor(mode='train')), 
        data_path='./.data/Lorenz/train/', 
        save_path='(Lorenz) KalmanNet.pt',
        mode=0)
    ...
    trainer_kalman_v2 = Trainer(
        dnn=KalmanNet_Filter_v2(
            LorenzAttractor(mode='train')), 
        data_path='./.data/Lorenz/train/', 
        save_path='(Lorenz, v2) KalmanNet.pt',
        mode=0)
    ...
    trainer_split = Trainer(
        dnn=Split_KalmanNet_Filter(
            LorenzAttractor(mode='train')), 
        data_path='./.data/Lorenz/train/', 
        save_path='(Lorenz) Split_KalmanNet.pt',
        mode=1)
            ...
            trainer_split.validate(
                Tester(
                        filter = Split_KalmanNet_Filter(
                            LorenzAttractor(mode='valid')), 
                        data_path = './.data/Lorenz/valid/',
                        model_path = './.model_saved/(Lorenz) Split_KalmanNet_' + str(trainer_split.train_count) + '.pt',
                        is_validation=True
                        )    
            )
            ...
            trainer_kalman.validate(
                Tester(
                        filter = KalmanNet_Filter(
                            LorenzAttractor(mode='valid')), 
                        data_path = './.data/Lorenz/valid/',
                        model_path = './.model_saved/(Lorenz) KalmanNet_' + str(trainer_kalman.train_count) + '.pt',
                        is_validation=True
                        )    
            )
            ...
            trainer_kalman_v2.validate(
                Tester(
                        filter = KalmanNet_Filter_v2(
                            LorenzAttractor(mode='valid')), 
                        data_path = './.data/Lorenz/valid/',
                        model_path = './.model_saved/(Lorenz, v2) KalmanNet_' + str(trainer_kalman_v2.train_count) + '.pt',
                        is_validation=True
                        )    
            )    
...
tester_ekf = Tester(
            filter = Extended_Kalman_Filter(
                LorenzAttractor(mode='test')), 
            data_path = './.data/Lorenz/test/',
            model_path = 'EKF'
            )   
...
for elem in test_list:
    ...
    tester_kf = Tester(
                filter = KalmanNet_Filter(
                    LorenzAttractor(mode='test')), 
                data_path = './.data/Lorenz/test/',
                model_path = './.model_saved/(Lorenz) KalmanNet_' + elem + '.pt'
                )
    ...
    tester_kf2 = Tester(
                filter = KalmanNet_Filter_v2(
                    LorenzAttractor(mode='test')), 
                data_path = './.data/Lorenz/test/',
                model_path = './.model_saved/(Lorenz, v2) KalmanNet_' + elem + '.pt'
                )
    ...
    tester_skf = Tester(
                filter = Split_KalmanNet_Filter(
                    LorenzAttractor(mode='test')), 
                data_path = './.data/Lorenz/test/',
                model_path = './.model_saved/(Lorenz) Split_KalmanNet_' + elem + '.pt'
                )
